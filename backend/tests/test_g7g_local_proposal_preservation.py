"""G7G: bounded local proposal preservation for qualified retrieval candidates."""
from __future__ import annotations

from backend.app.models import Evidence
from backend.app.rag.coverage_retrieval import (
    DEFAULT_RAW_OBSERVATION_K,
    _canonical_proposal_rank_key,
...
def test_ranked_fill_preserves_canonical_order_for_unqualified_tail():
    items = [
...
def test_promotion_does_not_evict_preserved_qualified_candidates():
    better_missing = _evidence(
        "family-a:10:20",
        "Julius Caesar crossed from Italy across the Adriatic into Epirus.",
        score=0.85,
        vector_rank=22,
    ...
    ranked = rerank_evidence(QUERY, [better_missing, preserved, *tail])
...
def test_movement_qualified_candidate_still_reaches_proposals():
    movement = _evidence(
        "move:0:80",
        "He marched from Brundisium across the Adriatic toward Epirus.",
        score=0.7,
        vector_rank=40,
        source="move-parent",
    )
    flood = [
        _evidence(
            f"flood:{index * 10}:{index * 10 + 80}",
            f"Julius Caesar marched from Italy toward Epirus in passage {index}.",
            score=0.95 - index * 0.001,
            vector_rank=index,
            source=f"flood-{index // 2}",
        )
        for index in range(1, 70)
    ]
    ranked = rerank_evidence(QUERY, [movement, *flood])
    preserved = select_qualified_local_proposals(ranked, DEFAULT_RAW_OBSERVATION_K)
    assert movement.id in {item.id for item in preserved}
    assert len({item.id for item in preserved}) == len(preserved)
